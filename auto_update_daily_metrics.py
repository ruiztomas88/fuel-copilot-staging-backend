"""
🔄 ACTUALIZADOR AUTOMÁTICO DE DAILY_TRUCK_METRICS
...
def update_daily_metrics():
    """Actualiza daily_truck_metrics desde fuel_metrics (últimas 24h)"""
    conn = pymysql.connect(**DB_CONFIG)
    try:
        cursor = conn.cursor()
        
        # Solo actualizar datos de HOY (evita recalcular histórico)
        cursor.execute("""
            ...
            FROM fuel_metrics
            WHERE DATE(timestamp_utc) >= CURDATE() - INTERVAL 1 DAY
        ...
        logger.info(f"✅ Updated {rows_affected} daily_truck_metrics records @ {datetime.now()}")
...
if __name__ == "__main__":
    logger.info("🚀 Starting auto-updater for daily_truck_metrics (every 10 min)")
