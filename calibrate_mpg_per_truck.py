...
def update_tanks_with_mpg(
    tanks_config: dict, mpg_by_truck: Dict[str, Dict[str, float]]
) -> dict:
    ...
    for truck_id, mpg_values in mpg_by_truck.items():
        ...
        if new_mpg:
            ...
            if old_mpg.get("overall") != new_mpg.get("overall"):
                updated_count += 1
                logger.info(
                    f"{truck_id}: MPG updated {old_mpg.get('overall', '?')} → {new_mpg.get('overall', '?')}"
                )
...
def main():
    ...
    parser.add_argument(
        "--update-inplace", action="store_true", help="Update tanks.yaml in place"
    )
    ...
    for truck_id, data in sorted(mpg_data.items()):
        ...
        print(
            f"{truck_id:<10} {str(highway):<10} {str(city):<10} {str(overall):<10} {samples:<10}"
        )
    ...
    if args.update_inplace or args.output:
        ...
        if output_path:
            with open(output_path, "w") as f:
                yaml.dump(
                    updated_config, f, default_flow_style=False, allow_unicode=True
                )
            ...
            print(
                yaml.dump(
                    {
                        "trucks": {
                            k: v for k, v in list(updated_config["trucks"].items())[:3]
                        }
                    },
