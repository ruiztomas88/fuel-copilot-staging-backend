...
def calculate_estimation_confidence(
    ...
    if battery_voltage is not None:
        if battery_voltage < 11.5:
            ...
            warnings.append(
                f"Batería baja ({battery_voltage:.1f}V < 11.5V) - sensores pueden fallar"
            )
    ...
    if sensor_age_seconds > 900:  # >15 min
        ...
        score -= age_penalty
        factors["sensor_age"] = (
            f"{sensor_age_seconds/60:.0f}min ({-age_penalty:.0f} pts)"
        )
    else:
    ...
    return EstimationConfidence(
        score=round(score, 1), level=level, factors=factors, warnings=warnings
    )
