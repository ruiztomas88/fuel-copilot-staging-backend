...
# ═══════════════════════════════════════════════════════════════════════════════
# 🆕 ENGINE HEALTH MONITORING FUNCTIONS - v3.13.0
# ═══════════════════════════════════════════════════════════════════════════════


def get_engine_health_data(
    truck_id: Optional[str] = None, minutes_back: int = 15
) -> List[Dict]:
    """
    Get latest engine health sensor data for trucks.

    Args:
        truck_id: Specific truck or None for all trucks
        minutes_back: How many minutes of data to consider "recent"

    Returns:
        List of dicts with sensor readings
    """
    try:
        engine = get_sqlalchemy_engine()
        ...
        if truck_id:
            ...
            query = text(
                ...
                INNER JOIN (
                    ...
                    FROM fuel_metrics
                    WHERE timestamp_utc >= DATE_SUB(NOW(), INTERVAL :minutes MINUTE)
                    GROUP BY truck_id
        ...
        logger.error(f"Error getting engine health data: {e}")
        return []


def get_engine_health_history(
    truck_id: str, days_back: int = 30, sensor: Optional[str] = None
) -> List[Dict]:
    """
    Get historical engine health data for trend analysis.

    Args:
        truck_id: Truck identifier
        days_back: Days of history to retrieve
        sensor: Optional specific sensor to filter
    ...
    try:
        ...
        sensors = "oil_pressure_psi, coolant_temp_f, oil_temp_f, battery_voltage, def_level_pct, engine_load_pct"
        if sensor:
            sensors = sensor
        ...
        with engine.connect() as conn:
            result = conn.execute(query, {"truck_id": truck_id, "days": days_back})
            rows = result.fetchall()
            columns = result.keys()
        ...
        logger.error(f"Error getting engine health history: {e}")
        return []


def save_engine_health_alert(alert_data: Dict) -> Optional[int]:
    """
    Save an engine health alert to the database.

    Args:
        alert_data: Dict with alert details

    Returns:
        Alert ID if saved, None if error
    """
    try:
        engine = get_sqlalchemy_engine()
        ...
        with engine.connect() as conn:
            result = conn.execute(query, alert_data)
            conn.commit()
            return result.lastrowid

    except Exception as e:
        logger.error(f"Error saving engine health alert: {e}")
        return None


def get_active_health_alerts(
    truck_id: Optional[str] = None, severity: Optional[str] = None
) -> List[Dict]:
    """
    Get active engine health alerts.

    Args:
        truck_id: Filter by truck (optional)
    ...
    try:
        ...
        logger.error(f"Error getting active health alerts: {e}")
        return []


def update_health_baselines(truck_id: str, baselines: Dict[str, Dict]) -> bool:
    """
    Update or insert baseline statistics for a truck.

    Args:
        truck_id: Truck identifier
        baselines: Dict of sensor_name -> baseline stats
    ...
    try:
        ...
        query = text(
            ...
            (truck_id, sensor_name, mean_30d, std_30d, min_30d, max_30d,
             mean_7d, std_7d, sample_count_30d, last_calculated)
            VALUES 
            (:truck_id, :sensor_name, :mean_30d, :std_30d, :min_30d, :max_30d,
             :mean_7d, :std_7d, :sample_count, NOW())
            ON DUPLICATE KEY UPDATE
                mean_30d = VALUES(mean_30d),
                std_30d = VALUES(std_30d),
                min_30d = VALUES(min_30d),
                max_30d = VALUES(max_30d),
                mean_7d = VALUES(mean_7d),
                std_7d = VALUES(std_7d),
                sample_count_30d = VALUES(sample_count_30d),
                last_calculated = NOW()
        """
        )

        with engine.connect() as conn:
            for sensor_name, stats in baselines.items():
                conn.execute(
                    query,
                    {
                        "truck_id": truck_id,
                        ...
                        "max_30d": stats.get("max_30d"),
                        "mean_7d": stats.get("mean_7d"),
                        "std_7d": stats.get("std_7d"),
        ...
        logger.error(f"Error updating health baselines: {e}")
        return False


def get_health_baselines(truck_id: str) -> Dict[str, Dict]:
    """
    Get baseline statistics for a truck.

    Args:
        truck_id: Truck identifier

    Returns:
        Dict of sensor_name -> baseline stats
    """
    try:
        engine = get_sqlalchemy_engine()
        ...
        query = text(
            """
            SELECT 
                sensor_name, mean_30d, std_30d, min_30d, max_30d,
                mean_7d, std_7d, sample_count_30d, last_calculated
        ...
        with engine.connect() as conn:
            result = conn.execute(query, {"truck_id": truck_id})
            rows = result.fetchall()
        ...
        for row in rows:
            baselines[row[0]] = {
                ...
                "mean_7d": float(row[5]) if row[5] else None,
                "std_7d": float(row[6]) if row[6] else None,
                "sample_count": int(row[7]) if row[7] else 0,
                "last_calculated": str(row[8]) if row[8] else None,
            }

        return baselines

    except Exception as e:
        logger.error(f"Error getting health baselines: {e}")
        return {}


def save_health_snapshot(truck_id: str, status_data: Dict) -> bool:
    """
    Save a health status snapshot for historical tracking.

    Args:
        truck_id: Truck identifier
        status_data: Health status data to save
    ...
    try:
        ...
        query = text(
            ...
            (truck_id, snapshot_time, overall_status,
             oil_pressure_psi, oil_pressure_status,
             coolant_temp_f, coolant_temp_status,
             oil_temp_f, oil_temp_status,
             battery_voltage, battery_status,
             def_level_pct, def_level_status,
             engine_load_pct, engine_load_status,
             rpm, alert_count_critical, alert_count_warning,
             oil_pressure_trend, coolant_temp_trend, battery_trend,
             data_age_minutes)
            VALUES 
            (:truck_id, NOW(), :overall_status,
             :oil_pressure_psi, :oil_pressure_status,
             :coolant_temp_f, :coolant_temp_status,
             :oil_temp_f, :oil_temp_status,
             :battery_voltage, :battery_status,
        ...
        with engine.connect() as conn:
            conn.execute(
                query,
                {
                    ...
                    "oil_pressure_psi": status_data.get("oil_pressure_psi"),
                    "oil_pressure_status": status_data.get("oil_pressure_status"),
                    "coolant_temp_f": status_data.get("coolant_temp_f"),
                    "coolant_temp_status": status_data.get("coolant_temp_status"),
                    "oil_temp_f": status_data.get("oil_temp_f"),
                    "oil_temp_status": status_data.get("oil_temp_status"),
                    "battery_voltage": status_data.get("battery_voltage"),
                    "battery_status": status_data.get("battery_status"),
                    "def_level_pct": status_data.get("def_level_pct"),
                    "def_level_status": status_data.get("def_level_status"),
                    "engine_load_pct": status_data.get("engine_load_pct"),
                    "engine_load_status": status_data.get("engine_load_status"),
                    "rpm": status_data.get("rpm"),
                    "alert_count_critical": status_data.get("alert_count_critical", 0),
                    "alert_count_warning": status_data.get("alert_count_warning", 0),
                    "oil_pressure_trend": status_data.get("oil_pressure_trend"),
                    "coolant_temp_trend": status_data.get("coolant_temp_trend"),
                    "battery_trend": status_data.get("battery_trend"),
                    "data_age_minutes": status_data.get("data_age_minutes", 0),
                },
            )
            conn.commit()
...
def get_fleet_health_stats(days: int = 7) -> Dict:
    """
    Get fleet-wide health statistics over time.

    Args:
        days: Number of days of history

    Returns:
        Dict with fleet health statistics
    """
    try:
        engine = get_sqlalchemy_engine()
        ...
        for row in rows:
            daily_stats.append(
                {
                    "date": str(row[0]),
                    "total_trucks": int(row[1]),
                    "healthy": int(row[2]),
                    "warning": int(row[3]),
                    "critical": int(row[4]),
                    "offline": int(row[5]),
                    "avg_critical_alerts": float(row[6] or 0),
                    "avg_warning_alerts": float(row[7] or 0),
                }
            )
        ...
        logger.error(f"Error getting fleet health stats: {e}")
        return {"days": days, "daily_stats": []}


def get_sensor_coverage_report() -> Dict:
    """
    Get report on which sensors are reporting data for which trucks.

    Returns:
        Dict with sensor coverage statistics
    """
    try:
        engine = get_sqlalchemy_engine()
        ...
        query = text(
            ...
            SELECT 
                ...
                SUM(CASE WHEN oil_pressure_psi IS NOT NULL THEN 1 ELSE 0 END) > 0 as has_oil_pressure,
                SUM(CASE WHEN coolant_temp_f IS NOT NULL THEN 1 ELSE 0 END) > 0 as has_coolant_temp,
                SUM(CASE WHEN oil_temp_f IS NOT NULL THEN 1 ELSE 0 END) > 0 as has_oil_temp,
                SUM(CASE WHEN battery_voltage IS NOT NULL THEN 1 ELSE 0 END) > 0 as has_battery,
                SUM(CASE WHEN def_level_pct IS NOT NULL THEN 1 ELSE 0 END) > 0 as has_def_level,
                SUM(CASE WHEN engine_load_pct IS NOT NULL THEN 1 ELSE 0 END) > 0 as has_engine_load
            FROM fuel_metrics
        ...
        for row in rows:
            truck = {
                "truck_id": row[0],
                "last_reading": str(row[1]) if row[1] else None,
                "sensors": {
                    "oil_pressure": bool(row[2]),
                    "coolant_temp": bool(row[3]),
                    "oil_temp": bool(row[4]),
                    "battery": bool(row[5]),
                    "def_level": bool(row[6]),
                    "engine_load": bool(row[7]),
                },
            }
            trucks.append(truck)
            ...
            # Count coverage
            if row[2]:
                coverage["oil_pressure"] += 1
            if row[3]:
                coverage["coolant_temp"] += 1
            if row[4]:
                coverage["oil_temp"] += 1
            if row[5]:
                coverage["battery"] += 1
            if row[6]:
                coverage["def_level"] += 1
            if row[7]:
                coverage["engine_load"] += 1

        return {
            "total_trucks": len(trucks),
            "coverage_counts": coverage,
            "coverage_pct": {
                sensor: round((count / len(trucks)) * 100, 1) if trucks else 0
                for sensor, count in coverage.items()
            },
            "trucks": trucks,
        }

    except Exception as e:
        logger.error(f"Error getting sensor coverage report: {e}")
        return {
            "total_trucks": 0,
            "coverage_counts": {},
            "coverage_pct": {},
