...
class DTCDecoder:
            ...
            with open(path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    try:
                        ...
                        # Get detailed explanation, fallback to description + notes
                        detailed_explanation = row.get("Detailed_Explanation", "")
                        if not detailed_explanation:
                            notes = row.get("Notes", "")
                            if notes:
                                detailed_explanation = f"{row.get('Description', f'Parameter {spn}')}. {notes}"
                            else:
                                detailed_explanation = row.get(
                                    "Description", f"Parameter {spn}"
                                )
    ...
    def decode_spn(self, spn: int) -> SPNInfo:
        ...
        # Unknown SPN - Enhanced OEM detection by range
        oem = "Unknown"
        category = "Unknown"
        description = f"Unknown SPN {spn}"

        # Standard J1939 range
        if 0 <= spn <= 519:
            oem = "J1939 Standard"
            category = "Standard"
            description = f"J1939 Standard Parameter {spn}"

        # Freightliner/Detroit Diesel (expanded range)
        elif (520000 <= spn <= 529999) or (3000 <= spn <= 3999):
            oem = "Freightliner/Detroit Diesel"
            category = "OEM Proprietary"
            description = f"Freightliner/Detroit Diesel Parameter {spn}"

        # Volvo (multiple ranges)
        elif (
            (600000 <= spn <= 609999)
            or (6000 <= spn <= 6999)
            or (80000 <= spn <= 84999)
            ...
            oem = "Volvo"
            category = "OEM Proprietary"
            description = f"Volvo Parameter {spn}"

        # Paccar (Kenworth/Peterbilt)
        elif (85000 <= spn <= 89999) or (4000 <= spn <= 4999):
            oem = "Paccar"
            category = "OEM Proprietary"
            description = f"Paccar (Kenworth/Peterbilt) Parameter {spn}"

        # Mack
        elif 90000 <= spn <= 94999:
            oem = "Mack"
            category = "OEM Proprietary"
            description = f"Mack Parameter {spn}"

        # International/Navistar
        elif (95000 <= spn <= 99999) or (100000 <= spn <= 104999):
            oem = "International/Navistar"
            category = "OEM Proprietary"
            description = f"International/Navistar Parameter {spn}"

        # Cummins
        elif (5000 <= spn <= 5999) or (110000 <= spn <= 119999):
            oem = "Cummins"
            category = "Engine"
            description = f"Cummins Parameter {spn}"

        # Caterpillar
        elif 2000 <= spn <= 2999:
            oem = "Caterpillar"
            category = "Engine"
            description = f"Caterpillar Parameter {spn}"

        # General system categorization for unknown SPNs
        else:
            if spn < 100:
                category = "Engine"
            elif 100 <= spn < 200:
        ...
        priority = 2  # Default HIGH priority for unknown SPNs
        if category == "Engine" or category == "Brakes":
            priority = 1  # CRITICAL
        elif category == "OEM Proprietary":
            priority = 2  # HIGH
        ...
        return SPNInfo(
            ...
            priority=priority,
            oem=oem,
            detailed_explanation=f"Parámetro {spn} de {oem}. Consultar manual del fabricante o dealer {oem} para diagnóstico específico.",
