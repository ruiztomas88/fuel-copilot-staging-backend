...
class EngineAnomalyDetector:
    ...
    def extract_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features['oil_press_normalized'] = df['oil_pressure_psi'].fillna(35) / rpm_factor
        ...
        features['temp_delta'] = df['oil_temp_f'].fillna(200) - df['coolant_temp_f'].fillna(180)
        ...
        if len(df) >= 5:
            features['pressure_stability'] = df['oil_pressure_psi'].fillna(35).rolling(
                window=min(5, len(df)), min_periods=1
            ).std().fillna(2)
        else:
        ...
        if len(df) >= 2:
            features['temp_rise_rate'] = df['oil_temp_f'].fillna(200).diff().fillna(0).clip(-10, 10)
        else:
    ...
    def predict(self, current_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        explanation = self._generate_explanation(anomaly_score, anomalous_features, status)
    ...
    def _generate_explanation(self, score: float, features: List[Dict], status: str) -> str:
        """Generate human-readable explanation."""
        ...
        if not features:
            return f"⚠️ Elevated anomaly score ({score}) but no specific feature identified"
        ...
        if status == 'WATCH':
            ...
            return f"⚠️ WARNING: Notable anomalies in {feature_list}. Consider inspection."
        else:
...
def get_fleet_anomaly_summary() -> Dict[str, Any]:
    ...
    scores = [r.get('anomaly_score', 0) for r in results if r.get('status') not in ['ERROR', 'INSUFFICIENT_DATA']]
    if scores:
