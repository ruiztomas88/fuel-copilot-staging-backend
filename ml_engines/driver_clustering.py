...
class DriverClusteringEngine:
    ...
    def extract_driver_features(self, driver_data: pd.DataFrame) -> pd.Series:
        ...
        if 'odometer_miles' in driver_data.columns:
            distance = driver_data['odometer_miles'].max() - driver_data['odometer_miles'].min()
        else:
        ...
        engine_hours = driver_data['engine_hours'].max() - driver_data['engine_hours'].min()
        ...
        features['avg_speed'] = moving_data['speed_mph'].mean() if len(moving_data) > 0 else 0
        
        # Feature 4: Speed consistency (lower std = more consistent)
        if len(moving_data) > 5:
            features['speed_consistency'] = 100 - min(moving_data['speed_mph'].std() * 2, 50)
        else:
    ...
    def fit(self, drivers_data: Dict[str, pd.DataFrame]) -> bool:
        ...
        if len(drivers_data) < self.n_clusters:
            logger.warning(f"Not enough drivers ({len(drivers_data)}) for {self.n_clusters} clusters")
            return False
    ...
    def predict_cluster(self, driver_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'metrics': {
                ...
                'fuel_efficiency_score': round(features.get('fuel_efficiency_score', 0), 1)
            }
        }
    ...
    def get_cluster_summary(self) -> Dict[str, Any]:
        ...
        for driver in all_drivers:
            ...
            for metric in clusters[key]['avg_metrics']:
                clusters[key]['avg_metrics'][metric].append(driver['metrics'].get(metric, 0))
        
        # Calculate averages
        for key in clusters:
            for metric in clusters[key]['avg_metrics']:
                values = clusters[key]['avg_metrics'][metric]
                clusters[key]['avg_metrics'][metric] = round(np.mean(values) if values else 0, 1)
...
def get_driver_data(truck_id: str, days: int = 30) -> pd.DataFrame:
    ...
    try:
        ...
        with engine.connect() as conn:
            df = pd.read_sql(text(query), conn, params={'truck_id': truck_id, 'days': days})
        return df
...
def _generate_fleet_insights(summary: Dict, drivers: List[Dict]) -> List[Dict]:
    ...
    if 'at_risk' in clusters and 'efficient_pro' in clusters:
        ...
        efficient_idle = clusters['efficient_pro'].get('avg_metrics', {}).get('idle_pct', 0)
        idle_diff = at_risk_idle - efficient_idle
        if idle_diff > 5:
            savings_estimate = round(idle_diff * at_risk.get('count', 0) * 50)  # $50 per % idle
            insights.append({
                'type': 'opportunity',
                'icon': '💰',
    ...
    for driver in result['drivers']:
        if driver['truck_id'] == truck_id:
            ...
            if cluster_key:
                ...
                driver['vs_cluster'] = {
                    'mpg_diff': round(driver['metrics']['avg_mpg'] - cluster_avg['avg_mpg'], 1),
                    'idle_diff': round(driver['metrics']['idle_pct'] - cluster_avg['idle_pct'], 1)
                }
