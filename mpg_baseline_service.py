...
class MPGBaselineService:
    ...
    def _calculate_from_readings(
        ...
        if not rows or len(rows) < self.MIN_SAMPLES:
            logger.info(f"[{truck_id}] Insufficient data: {len(rows) if rows else 0} samples")
        ...
        baseline.std_dev = statistics.stdev(filtered_values) if len(filtered_values) > 1 else 0.8
    ...
    def analyze_deviation(
        ...
        truck_id: str, 
        current_mpg: float,
        baseline: Optional[MPGBaseline] = None
    ) -> DeviationAnalysis:
        """
        ...
        if baseline is None:
            baseline = self._baselines_cache.get(
                truck_id, 
                MPGBaseline(truck_id=truck_id)
            )
        
        # Calculate deviation
        if baseline.baseline_mpg > 0:
            deviation_pct = ((current_mpg - baseline.baseline_mpg) / baseline.baseline_mpg) * 100
        else:
            deviation_pct = 0.0
        ...
        # Determine status
        status, message = self._get_deviation_status(z_score, deviation_pct, current_mpg)
    ...
    def _get_deviation_status(
        ...
        if z_score < -3.0 or current_mpg < 3.5:
            return "CRITICAL_LOW", f"MPG muy bajo ({current_mpg:.1f}) - revisar motor/sensores inmediatamente"
        
        if z_score > 3.0 or current_mpg > 10.0:
            return "CRITICAL_HIGH", f"MPG anormalmente alto ({current_mpg:.1f}) - posible error de sensor"
        
        # Anomaly thresholds
        if z_score < -2.0:
            return "LOW", f"MPG bajo ({deviation_pct:.0f}% debajo del baseline) - monitorear"
        
        if z_score > 2.0:
            return "HIGH", f"MPG alto ({deviation_pct:+.0f}% arriba del baseline) - verificar sensor"
    ...
    def get_fleet_summary(self) -> dict:
        ...
        high_confidence = [b for b in baselines if b.confidence in ("HIGH", "VERY_HIGH")]
        low_confidence = [b for b in baselines if b.confidence in ("LOW", "INSUFFICIENT")]
        
        avg_mpg = statistics.mean([b.baseline_mpg for b in baselines]) if baselines else 5.7
        ...
        return {
            ...
            "baselines": [b.to_dict() for b in sorted(baselines, key=lambda x: x.truck_id)],
        }
...
def compare_to_fleet_average(
    truck_baseline: MPGBaseline,
    fleet_average: float = 5.7
) -> dict:
    """
