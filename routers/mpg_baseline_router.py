...
router = APIRouter(prefix="/mpg-baseline", tags=["MPG Baseline"])
...
class MPGBaselineResponse(BaseModel):
    ...
    last_calculated: Optional[str] = Field(None, description="ISO timestamp of calculation")
...
class DeviationResponse(BaseModel):
    ...
    status: str = Field(..., description="NORMAL, LOW, HIGH, CRITICAL_LOW, CRITICAL_HIGH")
...
class CalculationRequest(BaseModel):
    ...
    truck_ids: Optional[list[str]] = Field(None, description="Specific trucks (or all if null)")
...
async def get_truck_baseline(
    ...
    try:
        ...
        logger.warning(f"Database not available, returning mock baseline for {truck_id}")
...
async def get_fleet_baselines(
    ...
    try:
        ...
        # Build response
        high_conf = sum(1 for b in baselines.values() if b.confidence in ("HIGH", "VERY_HIGH"))
        low_conf = sum(1 for b in baselines.values() if b.confidence in ("LOW", "INSUFFICIENT"))
        
        avg_mpg = sum(b.baseline_mpg for b in baselines.values()) / len(baselines) if baselines else 5.7
...
async def compare_to_fleet(
    ...
    try:
        ...
        if fleet_baselines:
            fleet_avg = sum(b.baseline_mpg for b in fleet_baselines.values()) / len(fleet_baselines)
        else:
            fleet_avg = 5.7
        ...
        if request.truck_ids:
            ...
            for truck_id in request.truck_ids:
                baseline = await service.calculate_baseline(
                    truck_id, 
                    days=request.days,
                    min_speed_mph=request.min_speed_mph
                )
