    ...
    if "from config import" in content:
        ...
        if import_type == "local" and "get_local_db_config" not in content:
            content = content.replace(
                "from config import", "from config import get_local_db_config,"
            )
        elif import_type == "wialon" and "get_wialon_db_config" not in content:
            content = content.replace(
                "from config import", "from config import get_wialon_db_config,"
            )
        elif import_type == "dual":
            if (
                "get_local_db_config" not in content
                or "get_wialon_db_config" not in content
            ):
                content = content.replace(
                    "from config import",
        ...
        content = re.sub(r"(import pymysql\n)", r"\1" + import_line, content, count=1)
