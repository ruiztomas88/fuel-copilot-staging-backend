...
def train_lstm_model(epochs=50, batch_size=32):
    ...
    try:
        ...
        history = predictor.train(
            df, epochs=epochs, batch_size=batch_size, validation_split=0.2
        )
...
def validate_models():
            ...
            test_data = pd.DataFrame(
                {
                    "timestamp_utc": pd.date_range(
                        start="2025-12-01", periods=30, freq="D"
                    ),
    ...
    try:
        ...
        if detector.load_model():
            ...
            print(
                f"   Test prediction: {result['risk_level']} (confidence: {result['confidence']:.2f})"
            )
        else:
