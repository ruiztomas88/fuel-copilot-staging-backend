...
class SensorHealthMonitor:
    ...
    def __init__(self, data_dir: str = "data"):
        ...
        # Estado en memoria
        self.sensor_history: Dict[str, List[Dict]] = {}  # sensor_key -> list of readings
        self.sensor_issues: Dict[str, List[SensorIssue]] = {}  # sensor_key -> list of issues
    ...
    def _load_state(self):
        ...
        try:
            if self.issues_file.exists():
                ...
                logger.info(f"Loaded sensor issues for {len(self.sensor_issues)} sensors")
    ...
    def _save_state(self):
        ...
        try:
            ...
            with open(self.issues_file, 'w') as f:
                json.dump(
                    {k: [asdict(issue) for issue in v] for k, v in self.sensor_issues.items()},
                    f, indent=2
                )
    ...
    def _detect_sensor_issues(
        ...
        if value == last_value:
            ...
            if same_value_count > 60:
                ...
                self.sensor_issues[sensor_key].append(issue)
                logger.warning(f"Sensor issue detected: {sensor_key} - stuck at {value}")
        
        # Issue 3: Erratic (cambio >20% en 1 lectura para fuel_pct)
        if sensor_name == "fuel_pct":
            pct_change = abs((value - last_value) / last_value * 100) if last_value > 0 else 0
            ...
            if pct_change > 20:
                ...
                self.sensor_issues[sensor_key].append(issue)
                logger.warning(f"Sensor issue detected: {sensor_key} - erratic change {pct_change:.1f}%")
        ...
        if out_of_range:
            ...
            self.sensor_issues[sensor_key].append(issue)
            logger.warning(f"Sensor issue detected: {sensor_key} - out of range {value}")
        
        # Limpiar issues antiguos (>7 días)
        cutoff = datetime.now() - timedelta(days=7)
        self.sensor_issues[sensor_key] = [
            issue for issue in self.sensor_issues[sensor_key]
    ...
    def get_sensor_health_report(
        ...
        if not history:
            return SensorHealthReport(
                ...
                issues_7d=0,
                recent_issues=[],
                recommendations=["Sensor nunca ha reportado datos - verificar conexión"]
            )
        
        # Calcular uptime (últimas 24 horas)
        cutoff_24h = datetime.now() - timedelta(hours=24)
        recent_readings = [r for r in history if datetime.fromisoformat(r['timestamp']) >= cutoff_24h]
        
        if recent_readings:
            valid_count = sum(1 for r in recent_readings if r['value'] is not None and r['is_valid'])
        ...
        issues_24h = sum(1 for i in issues if datetime.fromisoformat(i.timestamp) >= cutoff_24h)
    ...
    def _generate_recommendations(
        ...
        if health == SensorHealth.CRITICAL:
            recommendations.append(f"⚠️ URGENTE: {sensor_name} en estado crítico - requiere atención inmediata")
        elif health == SensorHealth.POOR:
            recommendations.append(f"⚠️ {sensor_name} en mal estado - considerar reemplazo pronto")
    ...
    def get_truck_sensor_health_summary(self, truck_id: str) -> Dict[str, SensorHealthReport]:
        """
