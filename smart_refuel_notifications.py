...
class SmartRefuelNotifier:
    ...
    def _load_state(self):
        ...
        try:
            if self.notifications_file.exists():
                ...
                logger.info(
                    f"Loaded {len(self.pending_notifications)} pending notifications"
                )
            ...
            if self.confirmations_file.exists():
                ...
                logger.info(
                    f"Loaded {len(self.confirmation_history)} confirmation records"
                )
    ...
    def _should_require_confirmation(
        self, gallons_added: float, increase_pct: float, confidence: float
    ) -> bool:
        """
    ...
    def get_pending_notifications(
        self, truck_id: Optional[str] = None, since_hours: float = 24
    ...
    def get_confirmation_stats(self, days: int = 30) -> Dict:
        ...
        recent_confirmations = [
            c
            for c in self.confirmation_history
            if datetime.fromisoformat(c["confirmed_at"]) >= cutoff
        ]

        if not recent_confirmations:
            return {"total": 0, "confirmed": 0, "rejected": 0, "accuracy": 0.0}
        ...
        return {
            ...
            "high_confidence_accuracy": (
                sum(1 for c in high_conf if c["is_valid"]) / len(high_conf) * 100
                if high_conf
                else 0
            ),
            "medium_confidence_accuracy": (
                sum(1 for c in medium_conf if c["is_valid"]) / len(medium_conf) * 100
                if medium_conf
                else 0
            ),
            "low_confidence_accuracy": (
                sum(1 for c in low_conf if c["is_valid"]) / len(low_conf) * 100
                if low_conf
                else 0
            ),
        }
