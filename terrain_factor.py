...
class TerrainTracker:
    ...
    def update(
        ...
        smoothed_altitude = sum(self.state.altitude_history) / len(self.state.altitude_history)
        
        # Calculate grade if we have previous position
        if self.state.last_altitude_ft is not None:
            # Calculate horizontal distance
            if latitude and longitude and self.state.last_latitude and self.state.last_longitude:
                horizontal_dist_ft = calculate_haversine_distance(
                    self.state.last_latitude, self.state.last_longitude,
                    latitude, longitude
                )
            ...
            if horizontal_dist_ft > self.config.min_distance_ft:
                ...
                if grade is not None:
                    ...
                    self.state.smoothed_grade_pct = (
                        alpha * grade + 
                        (1 - alpha) * self.state.smoothed_grade_pct
                    )
                    ...
                    self.state.distance_traveled_mi += horizontal_dist_ft / FEET_PER_MILE
        ...
        # Return fuel factor
        return calculate_terrain_fuel_factor(
            self.state.smoothed_grade_pct, 
            self.config
        )
    ...
    def get_terrain_summary(self) -> dict:
        ...
        return {
            ...
            "fuel_factor": round(calculate_terrain_fuel_factor(
    ...
    def get_fleet_summary(self) -> dict:
        ...
        for truck_id, tracker in self._trackers.items():
            ...
            if terrain == "CLIMBING":
                climbing.append({
                    "truck_id": truck_id,
                    "grade": tracker.state.smoothed_grade_pct
                })
