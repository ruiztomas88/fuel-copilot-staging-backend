...
class TestCacheService:
    ...
    async def test_invalidate_truck(self, cache_service):
        ...
        await cache_service.set(
            f"{cache_service.PREFIX_TRUCK}:VD3579:detail", {"data": 1}, 60
        )
        await cache_service.set(
            f"{cache_service.PREFIX_TRUCK}:VD3579:history", {"data": 2}, 60
        )
        await cache_service.set(
            f"{cache_service.PREFIX_FLEET}:summary", {"data": 3}, 60
        )
        ...
        assert (
            await cache_service.get(f"{cache_service.PREFIX_TRUCK}:VD3579:detail")
            is None
        )
        assert (
            await cache_service.get(f"{cache_service.PREFIX_TRUCK}:VD3579:history")
            is None
        )
