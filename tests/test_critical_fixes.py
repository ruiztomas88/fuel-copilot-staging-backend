...
class TestCircuitBreaker:
    ...
    def test_circuit_closed_allows_requests(self):
        ...
        breaker = CircuitBreaker(
            "test_closed", CircuitBreakerConfig(failure_threshold=3)
        )
    ...
    def test_circuit_opens_after_threshold(self):
        ...
        breaker = CircuitBreaker(
            "test_opens", CircuitBreakerConfig(failure_threshold=3)
        )
    ...
    def test_circuit_breaker_open_exception(self):
        ...
        breaker = CircuitBreaker(
            "test_exception", CircuitBreakerConfig(failure_threshold=2)
        )
    ...
    def test_circuit_closes_after_successful_recovery(self):
        ...
        breaker = CircuitBreaker(
            "test_closes",
            CircuitBreakerConfig(
                failure_threshold=2, success_threshold=2, timeout_seconds=1
            ),
        )
    ...
    def test_circuit_rejects_calls_when_open(self):
        ...
        breaker = CircuitBreaker(
            "test_rejects", CircuitBreakerConfig(failure_threshold=1)
        )
    ...
    def test_circuit_stats_tracking(self):
        ...
        breaker = CircuitBreaker(
            "test_stats", CircuitBreakerConfig(failure_threshold=5)
        )
    ...
    def test_circuit_get_status(self):
        ...
        breaker = CircuitBreaker(
            "test_status", CircuitBreakerConfig(failure_threshold=2)
        )
...
class TestReadingsPerDayValidation:
    ...
    def test_daily_data_default(self):
        ...
        history = [
            12.8 - (i * 0.01) for i in range(30)
        ]  # Declining voltage over 30 days
    ...
    def test_realistic_scenario_battery_voltage(self):
        ...
        if days_to_warning and days_to_warning > 0:
            assert (
                20 < days_to_warning < 60
            ), f"Expected ~37 days, got {days_to_warning}"
