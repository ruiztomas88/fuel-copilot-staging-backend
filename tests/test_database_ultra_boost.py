"""
Ultra boost para database_mysql.py - Objetivo: 70.9% → 90%
Necesita +19.1% de coverage (~300 líneas más)
Enfocado en funciones helper y edge cases
"""
...
class TestFleetSummary:
    ...
    def test_fleet_summary_empty_db(self, mock_engine):
...
class TestKPISummary:
    """Test get_kpi_summary variations"""
...
class TestLossAnalysis:
    """Test get_loss_analysis variations"""
...
class TestTruckEfficiency:
    ...
    @patch("database_mysql.get_sqlalchemy_engine")
    def test_efficiency_stats_30_days(self, mock_engine):
        """Test efficiency stats for 30 days"""
        mock_conn = MagicMock()
        mock_conn.execute.return_value.fetchall.return_value = []
        mock_engine.return_value.begin.return_value.__enter__.return_value = mock_conn
    ...
    @patch("database_mysql.get_sqlalchemy_engine")
    def test_efficiency_stats_7_days(self, mock_engine):
        """Test efficiency stats for 7 days"""
        mock_conn = MagicMock()
        mock_conn.execute.return_value.fetchall.return_value = []
        mock_engine.return_value.begin.return_value.__enter__.return_value = mock_conn
...
class TestDriverScoreHistory:
    ...
    @patch("database_mysql.get_sqlalchemy_engine")
    def test_ensure_driver_score_table(self, mock_engine):
...
class TestFuelRateAnalysis:
    ...
    def test_fuel_rate_48_hours(self, mock_engine):
        ...
        result = get_fuel_rate_analysis("TRK001", hours_back=48)
        assert isinstance(result, pd.DataFrame)

    @patch("database_mysql.get_sqlalchemy_engine")
    def test_fuel_rate_24_hours(self, mock_engine):
        """Test fuel rate analysis for 24 hours"""
        mock_conn = MagicMock()
        mock_result = MagicMock()
        mock_result.fetchall.return_value = []
        mock_conn.execute.return_value = mock_result
        ...
        result = get_fuel_rate_analysis("TRK002", hours_back=24)
        assert isinstance(result, pd.DataFrame)
...
class TestTruckHistory:
    ...
    def test_truck_history_168_hours(self, mock_engine):
        """Test truck history for 168 hours (7 days)"""
        mock_conn = MagicMock()
        mock_result = MagicMock()
        mock_result.fetchall.return_value = []
        mock_conn.execute.return_value = mock_result
...
class TestRefuelHistory:
    ...
    @patch("database_mysql.get_sqlalchemy_engine")
    def test_refuel_history_with_dates(self, mock_engine):
        """Test refuel history with date range"""
        mock_conn = MagicMock()
        mock_result = MagicMock()
        mock_result.mappings.return_value.all.return_value = []
        mock_conn.execute.return_value = mock_result
...
class TestLatestTruckData:
    ...
    def test_latest_truck_data_48_hours(self, mock_engine):
        ...
        result = get_latest_truck_data(hours_back=48)
        assert isinstance(result, pd.DataFrame)
...
class TestEdgeCases:
    ...
    @patch("database_mysql.get_sqlalchemy_engine")
    def test_negative_days_back(self, mock_engine):
        """Test functions with negative days_back"""
        mock_conn = MagicMock()
        mock_conn.execute.return_value.fetchall.return_value = []
        mock_engine.return_value.begin.return_value.__enter__.return_value = mock_conn
    ...
    @patch("database_mysql.get_sqlalchemy_engine")
    def test_very_large_days_back(self, mock_engine):
        """Test functions with very large days_back"""
        mock_conn = MagicMock()
        mock_conn.execute.return_value.fetchall.return_value = []
        mock_engine.return_value.begin.return_value.__enter__.return_value = mock_conn
    ...
    def test_empty_response_consistency(self):
        """Test that all empty responses are consistent"""
        fleet = _empty_fleet_summary()
        kpi = _empty_kpi_response(3.50)
        loss = _empty_loss_response(1, 3.50)
        enhanced = _empty_enhanced_kpis(1, 3.50)
