...
class TestMaintenancePredictionEndpoints:
    ...
    def test_predict_maintenance_success(
        self, mock_predictor, client, mock_db_connection
    ):
    ...
    @patch("routers.ml.get_maintenance_predictor")
    def test_predict_maintenance_insufficient_data(
        self, mock_predictor, client, mock_db_connection
    ):
    ...
    @patch("routers.ml.get_maintenance_predictor")
    def test_predict_fleet_maintenance(
        self, mock_predictor, client, mock_db_connection
    ):
        ...
        mock_instance.predict_truck.side_effect = [
            {
                "maintenance_7d_prob": 0.85,
                "maintenance_14d_prob": 0.60,
                "maintenance_30d_prob": 0.30,
                "recommended_action": "urgent_maintenance",
                "confidence": "high",
            },
            {
                "maintenance_7d_prob": 0.15,
                "maintenance_14d_prob": 0.10,
                "maintenance_30d_prob": 0.05,
                "recommended_action": "normal_operation",
                "confidence": "medium",
            },
            {
                "maintenance_7d_prob": 0.50,
                "maintenance_14d_prob": 0.35,
                "maintenance_30d_prob": 0.15,
        ...
        # Make request
        response = client.get(
            "/fuelAnalytics/api/ml/maintenance/fleet-predictions?top_n=3"
        )
...
class TestTheftDetectionEndpoints:
    ...
    def test_recent_theft_predictions(self, mock_detector, client, mock_db_connection):
        ...
        # Make request
        response = client.get(
            "/fuelAnalytics/api/ml/theft/recent-predictions?hours=24&min_confidence=0.5"
        )
...
class TestTrainingEndpoint:
    ...
    def test_train_both_models(
        ...
        mock_db_connection.fetchall.side_effect = [
            ...
            # Theft training data
            [("DO9693", "2025-01-01", 30.0, 5, 12, 1.0, 34.05, -118.25, "STOPPED", 0)]
            * 500,
        ]
        ...
        assert (
            "lstm" in data["models_trained"]
            or "isolation_forest" in data["models_trained"]
        )

    def test_train_invalid_model(self, client):
        """Test training with invalid model name"""
        response = client.post(
            "/fuelAnalytics/api/ml/train", json={"model": "invalid_model"}
        )
...
class TestStatusEndpoint:
    ...
    @patch("routers.ml.get_maintenance_predictor")
    @patch("routers.ml.get_theft_detector")
    def test_status_models_loaded(
        self, mock_theft_detector, mock_maintenance_predictor, client
    ):
    ...
    @patch("routers.ml.get_maintenance_predictor")
    @patch("routers.ml.get_theft_detector")
    def test_status_models_not_loaded(
        self, mock_theft_detector, mock_maintenance_predictor, client
    ):
