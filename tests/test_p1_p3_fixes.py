...
try:
    ...
    engine.process_reading(
        truck_id="ACTIVE001", timestamp=now - timedelta(hours=1), speed=60.0, rpm=1500
    )

    # Inactive truck (8 days old)
    engine.process_reading(
        truck_id="INACTIVE001", timestamp=now - timedelta(days=8), speed=50.0, rpm=1400
    )

    # Another inactive truck (10 days old)
    engine.process_reading(
        truck_id="INACTIVE002", timestamp=now - timedelta(days=10), speed=55.0, rpm=1450
    )
    ...
    assert (
        len(engine.truck_states) == 1
    ), f"Should have 1 truck remaining, have {len(engine.truck_states)}"
...
print(
    f"{'✅' if bare_except_count == 0 else '❌'} Exception Handling: {'PASS' if bare_except_count == 0 else 'FAIL'}"
)
