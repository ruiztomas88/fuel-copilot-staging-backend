...
class TestVoltageThresholds:
    ...
    def test_thresholds_defined(self):
        ...
        required = ["critical_low", "low", "normal_min", "normal_max", "high", "critical_high"]
        for key in required:
...
class TestVoltageTrendingService:
    ...
    def test_calculate_stats_with_engine_states(self):
        ...
        now = datetime.utcnow()
        points = [
            VoltageDataPoint(timestamp=now - timedelta(hours=2), voltage=12.5, engine_running=False),
            VoltageDataPoint(timestamp=now - timedelta(hours=1), voltage=14.2, engine_running=True),
    ...
    def test_detect_anomalies(self):
        ...
        points = [
            ...
            VoltageDataPoint(timestamp=now - timedelta(hours=3), voltage=11.0),  # Critical low
            VoltageDataPoint(timestamp=now - timedelta(hours=2), voltage=16.0),  # Critical high
            VoltageDataPoint(timestamp=now - timedelta(hours=1), voltage=13.5),
...
class TestVoltageHistoryResponse:
    ...
    def test_to_dict_complete(self):
        ...
        response = VoltageHistoryResponse(
            ...
            period_hours=24,
            data_points=[
                VoltageDataPoint(timestamp=now, voltage=13.5)
            ],
            stats=VoltageStats(
                truck_id="TEST001",
                period_hours=24,
