...
try:
    ...
    print("📋 BEFORE UPDATE:")
    print("-" * 80)
    cursor.execute("SELECT beyondId, unit FROM units_map WHERE beyondId IN ('FF7702', 'JB6858', 'RT9127')")
    ...
    for truck_id, unit_id in updates.items():
        # Check if exists
        cursor.execute("SELECT COUNT(*) as cnt FROM units_map WHERE beyondId = %s", (truck_id,))
        exists = cursor.fetchone()['cnt'] > 0
        
        if exists:
            # Update
            cursor.execute("UPDATE units_map SET unit = %s WHERE beyondId = %s", (unit_id, truck_id))
